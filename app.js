//https://api.openweathermap.org/data/2.5/weather?q={city name}&appid={API key}

let weather = {
  apiKey: "3001c7c2e21157b59323465887c290c5",
  fetchWeather: function (city) {
    fetch(
      "https://api.openweathermap.org/data/2.5/weather?q=" +
        city +
        "&units=metric&appid=" +
        this.apiKey
    )
      .then((response) => {
        if (!response.ok) {
          listOfCities.innerHTML = "";
          alert("No such city...");
          throw new Error("No weather found.");
        }
        return response.json();
      })
      .then((data) => this.displayWeather(data));
  },
  displayWeather: function (data) {
    const { name } = data;
    const { icon, description } = data.weather[0];
    const { temp, humidity } = data.main;
    const { speed } = data.wind;
    document.querySelector(".city").innerText = "Weather in " + name;
    document.querySelector(".icon").src =
      "https://openweathermap.org/img/wn/" + icon + ".png";
    document.querySelector(".description").innerText = description;
    document.querySelector(".temp").innerText = "Temperature: " + temp + "°C";
    document.querySelector(".humidity").innerText =
      "Humidity: " + humidity + "%";
    document.querySelector(".wind").innerText =
      "Wind speed: " + speed + " km/h";
    document.querySelector(".weather").classList.remove("loading");
    document.body.style.backgroundImage =
      "url('https://source.unsplash.com/1600x900/?" + name + "')";
  },
  search: function () {
    this.fetchWeather(document.querySelector(".search-bar").value);
  },
};

document.querySelector(".search button").addEventListener("click", function () {
  weather.search();
  document.querySelector(".search-bar").value = null;
  listOfCities.innerHTML = "";
});

weather.fetchWeather("pune");

document.getElementById("clear").addEventListener("click", function () {
  document.querySelector(".search-bar").value = null;
  listOfCities.innerHTML = "";
});

const search = document.getElementById("searchbar");
const listOfCities = document.getElementById("cityList"); //displayed

const searchCity = async (searchText) => {
  const res = await fetch("./cities.json");
  const data = await res.json();

  let matches = data.filter((city) => {
    const regx = new RegExp(`^${searchText}`, "gi");
    return city.name.match(regx);
  });

  if (searchText.length == 0) {
    matches = [];
    listOfCities.innerHTML = "";
  }

  outputHtml(matches);
};

const outputHtml = (matches) => {
  if (matches.length > 0) {
    const html = matches
      .map(
        (match) => `
      <div>${match.name}</div>
    `
      )
      .join("");
    listOfCities.innerHTML = html;
    listOfCities.addEventListener("click", function (event) {
      if (
        event.path[0].textContent.length < 15 &&
        event.path[0].textContent.length > 0
      ) {
        searchbar.value = event.path[0].textContent;
        weather.search();
        listOfCities.innerHTML = "";
        document.querySelector(".search-bar").value = null;
      }
    });
  }
};

search.addEventListener("input", () => {
  searchCity(search.value);
});
